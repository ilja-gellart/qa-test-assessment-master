...
var searchBox = element(by.id('query'));
var searchBtn = element(by.css('.btn'));
var cardElem = element(by.css('.card'));
var notFoundTxt = element(by.cssContainingText('div', 'Not found'));
var personRadioBtn = element(by.id('people'));
var planetsRadioBtn = element(by.id('planets'));
...
When('User select a tab {string}', { timeout: 25 * 1000 }, async (string) => {
  if (string == "Person")
...
When('User search for {string} character', { timeout: 25 * 1000 }, async (string) => {
  await searchBox.sendKeys(string);
  await searchBtn.click();
});

When('User clear search box and hit Enter key', { timeout: 25 * 1000 }, async () => {
  await searchBox.clear();
  await searchBox.sendKeys(protractor.Key.ENTER);
});

When('User click on search button', { timeout: 25 * 1000 }, async () => {
  await searchBtn.click();
});

Then('Verify it’s a Not Found result', { timeout: 25 * 1000 }, async () => {
  await chai.expect(notFoundTxt.isDisplayed()).to.eventually.be.true;
});

Then('Verify every valid result for Person contains his / her “Gender”, “Birth year”, “Eye color” and “Skin color', { timeout: 25 * 1000 }, async () => {
